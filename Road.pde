...
class Road {
  ...
  // Constructor
  Road(int ypos_, int whiteLine, int yellowLines, int numCars_, int carDirection_) {
    ypos = ypos_;
  ...
  void display() {
    ...
    if (roadImage == null) {
      ...
      float ratio = roadImage.height/(height/10);
      for (int x=0; x<width; x += roadImage.width/ratio) {
        image(roadImage, x, ypos, roadImage.width/ratio, height/10);
      }
    }
    ...
    if (showWhiteLine == 1) {
      ...
      noStroke();
      fill(255);
      rect(0, ypos+height/10-10, width, 5);
    }
    ...
    if (showYellowLines == 1) {
      noStroke();
      fill(255, 255, 0);
      rect(0, ypos+height/20-10, width, 5);
      rect(0, ypos+height/20+5, width, 5);
    }
  }
...
class Car {
  ...
  float maxSizeX = 100;
  int maxSizeY = height/11;

  float minSizeX = 30;
  int minSizeY = height/20;
