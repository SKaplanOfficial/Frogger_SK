...
class Scene {
  ...
  void loadData() {
    ...
    // Layout array is 10 rows long and 11 columns across
    ...
    for (int y=0; y<10; y++) {
      for (int x=0; x<11; x++) {
        ...
        if (layout[pos].equals("E")) {                         // End row
          safezones.add(new Safezone(y*(height/10)));
          ...
          if (layout[1+y*11].equals("-1")) {                   // Normal road
            roads.add(new Road(y*(height/10), 0, 0, numberOfCars, directionOfCars));
            roadAmount++;
          } else if (layout[1+y*11].equals("1")) {             // Upper white line
            roads.add(new Road(y*(height/10), 1, 0, numberOfCars, directionOfCars));
            roadAmount++;
          } else if (layout[1+y*11].equals("2")) {             // Lower white line
            roads.add(new Road(y*(height/10), 2, 0, numberOfCars, directionOfCars));
            roadAmount++;
          } else if (layout[1+y*11].equals("3")) {             // Middle yellow lines
            roads.add(new Road(y*(height/10), 0, 1, numberOfCars, directionOfCars));
            roadAmount++;
          }
          
        } else if (layout[pos].equals("W")) {                  // Water/River row
          rivers.add(new River(y*(height/10)));
          riverAmount++;
          
        } else if (layout[pos].equals("P")) {                  // Power Ups (At a specific spot)
          powerUps.add(new PowerUp(x*(width/10), y*(height/10)));
          powerUpAmount++;
          
        } else if (layout[pos].equals("F")) {                  // Frogger (At a specific spot)
          frogger = new Frog(x*(width/10), y*(height/10));
        }
      }
